...
class CrudCategorias(QDialog):
    ...
    def setup_ui(self):
        ...
        self.table.setColumnCount(2) # ID Column removed
        self.table.setHorizontalHeaderLabels(["Nombre", "Tipo Global"])
    ...
    def load_data(self):
        try:
            ...
            for row_idx, row_data in enumerate(registros):
                ...
                self.table.setItem(row_idx, 1, QTableWidgetItem(row_data['tipo']))
    ...
    def seleccionar_registro(self):
        ...
        row = selected[0].row()
        self.current_categoria_id = self.table.item(row, 0).data(Qt.UserRole)
        
        self.le_nombre.setText(self.table.item(row, 0).text())
        self.cb_tipo.setCurrentText(self.table.item(row, 1).text())
